...
// Options for dropdown/radio fields are stored as a JSON array string.
function parseOptions(raw) {
  if (!raw) return [];
  try { const a = JSON.parse(raw); return Array.isArray(a) ? a : []; } catch { return []; }
}

// originalBytes: the source PDF (already decrypted by the caller's file store);
// falls back to reading document.file_path directly for plain files.
export async function buildFinalPdf({ document, originalBytes = null, recipients, fields, certInfo = null }) {
  const bytes = originalBytes || (await fs.readFile(document.file_path));
  ...
  for (const f of fields) {
    ...
    if (f.type === 'checkbox') {
      ...
      // Bordered box showing the chosen option as text.
      page.drawRectangle({ x, y: yBottom, width: w, height: h, borderColor: RULE, borderWidth: 1 });
      if (f.value) {
        const size = Math.min(h * 0.6, 12);
        page.drawText(String(f.value), { x: x + 5, y: yBottom + (h - size) / 2 + 1, size, font: helv, color: INK });
      }
    } else if (f.type === 'radio') {
      // One row per option: a circle (filled for the chosen one) plus its label.
      const opts = parseOptions(f.options);
      const rowH = Math.min(18, h / Math.max(1, opts.length));
      const r = Math.max(2.5, Math.min(5, rowH * 0.32));
      const size = Math.min(rowH * 0.62, 11);
      opts.forEach((o, i) => {
        const cy = yBottom + h - rowH * (i + 0.5); // rows top-to-bottom
        const cx = x + r + 2;
        page.drawCircle({ x: cx, y: cy, size: r, borderColor: INK, borderWidth: 0.8 });
        if (String(f.value) === String(o)) {
          page.drawCircle({ x: cx, y: cy, size: r * 0.5, color: INK });
        }
        page.drawText(String(o), { x: cx + r + 5, y: cy - size / 2 + 1, size, font: helv, color: INK });
      });
...
async function appendCertificate(pdf, { document, recipients, helv, helvBold, certInfo }) {
  ...
  for (const r of recipients) {
    ...
    if (r.access_code_hash) {
      text(`Access code verified:  ${fmt(r.code_verified_at)}`, { color: MUTED });
    }
